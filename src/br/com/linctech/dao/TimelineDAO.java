...
public class TimelineDAO {
	...
	public List<Timeline> selecionarTempo() throws SQLException {
		...
		while (result.next()) {
			lista.add(new Timeline(result.getString("ACONTECIMENTO"), 
					result.getDate("DT_TIMELINE").toLocalDate(),
					result.getString("BREVE_DESCRICAO"),result.getString("TEMPO"),
					new Personagem(result.getInt("ID_PERSONAGEM"), result.getString("NOME"), result.getString("DESCRICAO"),
	...
	public List<Passado> selecPassado(String tempo) throws SQLException {
	...
	public List<Timeline> selecFuturo() throws SQLException {
		List<Timeline> lista = new ArrayList<Timeline>();
